...
set.seed(3457)
m_rf_r <-randomForest(as.factor(BD$RETORNADO_CATEGORIA) ~ BD$COMPROMISOS + BD$SATISFACCION+BD$PERSONAL+BD$VOLVER.A.CONTRATAR+BD$PRODUCTOS+BD$SOPORTE_ADMON+BD$SOPORTE_DIR+BD$SOPORTE_TEC, importance=TRUE,ntree=100000)
...
m_bor_r <-Boruta(as.factor(BD$RETORNADO_CATEGORIA) ~ BD$COMPROMISOS + BD$SATISFACCION+BD$PERSONAL+BD$VOLVER.A.CONTRATAR+BD$PRODUCTOS+BD$SOPORTE_ADMON+BD$SOPORTE_DIR+BD$SOPORTE_TEC, doTrace=0,maxRuns=500)
j= 1
#print(c("CORRIDA",i))
...
write.csv(bortotr, file = "E:/PROYECTO/Documentos/PAYC/PROYECTO 10. REPLANIFICACIÓN PAYC/ANALISIS/OTRO SÍS, LA REC. DE CLI. Y LA SAT.  DE LOS CLI/SCRIPTS/BORUTA_RETORNADOS_V3.csv")
