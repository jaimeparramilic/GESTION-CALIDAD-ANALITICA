...
DATOS<-read.csv("DATA.CSV", header = T, sep = ";")
DATOS<-as.data.frame(DATOS)
DATOS$FECHA.INICIO.F1<-as.Date(DATOS$FECHA.INICIO.F1, "%d/%m/%Y")
DATOS$FECHA.FIN.F1<-as.Date(DATOS$FECHA.FIN.F1, "%d/%m/%Y")

#AJUSTE DE DISTRIBUCIÓN --------------------------------
INFO<-DATOS$DIF_TRASLA[!is.na(DATOS$DIF_TRASLA)]
plotdist(INFO)
fit_n <- fitdist(INFO, "norm")
fit_l <- fitdist(INFO, "lnorm")
fit_e <- fitdist(INFO, "exp")
fit_u <- fitdist(INFO, "unif")
fit_w <- fitdist(INFO, "weibull")
fit_g <- fitdist(INFO, "gamma")

plot.legend <- c("Normal","Lognormal", "Uniforme", "Exponencial","Weibull","Gamma")
funciones<-list(fit_n,fit_l, fit_u, fit_e,fit_w,fit_g)

denscomp(funciones, legendtext = plot.legend)
cdfcomp (funciones, legendtext = plot.legend)
qqcomp  (funciones, legendtext = plot.legend)
ppcomp  (funciones, legendtext = plot.legend)

tnorm <- ks.test(INFO, "pnorm")
tlnorm <- ks.test(INFO, "plnorm")
tuni <- ks.test(INFO, "punif")
texp <- ks.test(INFO, "pexp")
tweib <- ks.test(INFO, "pweibull", shape=fit_w$estimate["shape"], scale = fit_w$estimate["scale"])
tgamm <- ks.test(INFO, "pgamma", shape = fit_g$estimate["shape"], rate = fit_g$estimate["rate"])

denscomp(list(fit_n),main="NORMAL")
denscomp(list(fit_l),main="LOGNORMAL")
denscomp(list(fit_u),main="UNIFORME")
denscomp(list(fit_e),main="EXPONENCIAL")
denscomp(list(fit_w),main="WEIBULL")
denscomp(list(fit_g),main="GAMMA")
...
PRUEBA<-FASE2_SIMULACION[FASE2_SIMULACION$ACUERDO.DE.OBRA==201001,]
PRUEBA<-aggregate(VALOR_PAGADO~CORRIDA+FECHA_PAGO+VALOR, data=PRUEBA, FUN="sum")
PRUEBA$VALOR_PAGADO_PROMEDIO<-PRUEBA$VALOR_PAGADO/Corridas
